...
- **site-to-site:** каждый `vpn_cidr` (LAN пира) реально маршрутизируется в
  активный туннель. Проверка пассивная — `ip route get <addr> mark <wg-марка>`;
  FIXABLE, если egress-устройство не активный туннель (нужен `wg-split-apply`).
